...
def get_latest_launch_template(ec2_client: boto3.client) -> Any:
    try:
        ...
        logger.error(
            f"Error fetching launch template {get_launch_template_name()}, please make sure that the launch template exist",
            error,
        )
        raise Exception(
            f"Error fetching launch template {get_launch_template_name()}, please make sure that the launch template exist",
    ...
    with Halo(text="Creating Launch Template", spinner="dots"):
        ...
        try:
            ec2_response = ec2_client.create_launch_template(
                LaunchTemplateName=get_launch_template_name(),
                ...
                TagSpecifications=[
                    {
                        ...
                        "Tags": [
                            {"Key": "Name", "Value": get_launch_template_name()},
                            {"Key": "Owner", "Value": username},
                        ],
                    },
                ],
